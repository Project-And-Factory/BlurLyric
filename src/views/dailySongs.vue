<template>
  ...
  </h2> <a @click="app.changeTrack({tracks: app.data.recommendSongs.data.dailySongs,num : 0})" class="linkbox"><a>播放全部</a></a><br>
  <div style="clean: both" class="track" v-if="app.data.recommendSongs.data">
        <div @click="setTracks(app.data.recommendSongs.data.dailySongs,i)" class="tracks" :muid="item.id" v-for="(item,i) in app.data.recommendSongs.data.dailySongs" :key="item.id">
            <div class="infor">
            ...
            query: {
              id: name.id
                ...
                <div v-if="app.data.recommendSongs.data.recommendReasons.find((reason)=>{return reason.songId == item.id})" class="trackAl">
        ...
        setTracks(track,num){
          let tempTime = new Date().getTime()
                if ((tempTime - time) < 500) {
                    time = tempTime - 1000
                    app.changeTrack({tracks: track,num : num})
                }
                time = tempTime
        },
    },
